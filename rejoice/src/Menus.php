...
class Menus implements \ArrayAccess
{
    ...
    {
        foreach ($modifications as $menu_id => $modif) {
            if (isset($modif[MSG])) {
                $this->menus[$menu_id][MSG] = $modif[MSG];
            }

            if (isset($modif[ACTIONS])) {
                $this->modifyPageActions($modif[ACTIONS], $menu_id);
            }
        }
    }

    public function modifyPageActions($modifications, $menu_id)
    {
        if (!isset($this->menus[$menu_id][ACTIONS])) {
            $this->menus[$menu_id][ACTIONS] = [];
        }

        foreach ($modifications as $key => $value) {
            $this->menus[$menu_id][ACTIONS][$key] = $value;
        }
    }
    ...
    public function getNextMenuId(
        ...
        if ($user_response_exists_in_menu_actions) {
            return $this->menus[$menu_id][ACTIONS][$user_response][ITEM_ACTION];
        ...
        } elseif (isset($this->menus[$menu_id][ACTIONS][DEFAULT_MENU_ACTION])) {
            return $this->menus[$menu_id][ACTIONS][DEFAULT_MENU_ACTION];
